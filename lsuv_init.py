...
from tensorflow.keras.layers import Dense, Conv1D, TimeDistributed
...
def LSUVinit(model, batch, verbose=True, margin=0.1, max_iter=10):
    ...
    classes_to_consider = (Dense, Conv1D, TimeDistributed)
    needed_variance = 1.0
    ...
    for layer in model.layers:
        ...
        while abs(needed_variance - variance) > margin:
            ...
            weights_and_biases = layer.get_weights()
            for i in range(len(weights_and_biases)-1) :
                weights_and_biases[i] /= np.sqrt(variance) / np.sqrt(needed_variance)
            layer.set_weights(weights_and_biases)
